...
- 浏览/检索：Home 搜索（标题/简介）+ 可点标签筛选 + 最新/最热排序 + 游标分页；`GET /api/games/:id` 详情 API + **详情页 `/games/:id`**（封面/简介/标签/统计/玩法提示/立即游玩）。
- 玩法提示（T1）：`controls` 从 manifest 透传进 active-version 响应，Play 页（加载态 + 底部）与详情页向玩家展示键位。
- 运行时埋点：Play 回写 `PlayEvent`（LOAD/END/ERROR）→ `playCount` 自增（LOAD）；Create run-timeline 聚合并展示生成成本（6 节点 token）。
- 版本管理 + 作者管理（owner-scoped）：Play/详情页「＋ 新版本」→ `/create?gameId=` 产 `versionNumber+1`（发布即切 active）；详情页作者面板改 meta / 下架(ARCHIVED) / 删除（级联 + 清 MinIO 产物，删除封装只在 `storage.ts`）。
- 安全增强：per-user 任务频率限额 + 公开埋点 `/api/play-events` 节流（Redis 固定窗口，429，fail-open）；失效会话存在性再校验（401 而非 500）；详情页/接口非作者不可见草稿（404）；owner-scoped 越权 403。
...
**再给一周**：真接 GPT-5.5 跑真实质量产出 + ingest 喂 imageUrl；Remix；点赞/收藏写入 + UI；平台 admin（role + 跨用户管理/可观测）+ 内容审核；durable-step 续跑；OAuth 真跑通其一。
