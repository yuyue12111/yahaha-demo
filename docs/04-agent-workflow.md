...
| `CODER` | `{ spec, assetPlan }` | `{ files: {path,content}[] }` | 产出自包含 HTML5 游戏（满足运行时契约：挂载 `#game-root` + postMessage 生命周期）。**两种实现（`CODER_MODE`，2026-06-21）**：① **`template`（默认）—— 秒级确定性引擎**：模型仍参与 PLANNER 出 spec（配色/标题/玩法随输入变），game.js 用确定性模板（验收快稳、离线复现、红线③确定性）。② `auto`/`llm` —— 真模型走 **LLM code-gen**：GPT-5.5 据创意+spec **真写整个 game.js**（复杂/多样游戏如跑酷，但慢 ~1–2min，**仅 demo 用**），经语法+协议+体积校验，失败/超时**回退模板**。模型生成代码在跨域 sandbox(红线②)里跑，碰不到宿主。 |
