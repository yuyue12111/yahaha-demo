...
export function AuthForm({ mode }: { mode: "login" | "register" }) {
  ...
  return (
    <form onSubmit={onSubmit} className="flex flex-col gap-3">
      ...
      {/* OAuth 占位（A1）：流程设计已就绪（docs/07 §OAuth），demo 暂未接入 → disabled + tooltip。 */}
...
/** OAuth 第三方登录占位按钮（A1）：disabled + tooltip，呼应 docs/07 OAuth 设计。 */
function OAuthButton({ label, children }: { label: string; children: ReactNode }) {
  return (
    <button
      type="button"
      disabled
      title="设计已就绪（docs/07），demo 暂未接入"
      aria-label={`使用 ${label} 登录（暂未接入）`}
      className="flex h-10 flex-1 cursor-not-allowed items-center justify-center gap-2 rounded-lg border border-hairline-strong text-[13px] font-medium text-ink-muted opacity-60"
    >
      {children}
      {label}
    </button>
